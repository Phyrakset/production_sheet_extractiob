...
const OrderBlock = ({ order, isLast }) => {
  ...
  const renderSizeBreakdowns = () => {
    ...
    // Custom sort order for PTCOC270 sizes (XS, S, M, L, XL, XXL)
    ...
    <div className="avoid-break" style={{ padding: '40px', backgroundColor: 'white', color: 'black', fontFamily: '"Times New Roman", Times, serif', minHeight: '100%', position: 'relative', marginBottom: isLast ? '0' : '40px', pageBreakAfter: isLast ? 'auto' : 'always' }}>
      ...
      <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px', width: '100%' }}>
          ...
          <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse', marginTop: '10px' }}>
            <tbody>
              ...
              <tr>
                ...
                <td style={{ padding: '1px 0' }}>: {header?.customerStyle || 'STCO4143 (STCO4143)'}</td>
              </tr>
              <tr>
                <td style={{ padding: '1px 0' }}>QUANTITY</td>
                <td style={{ padding: '1px 0' }}>: {header?.quantity || ''}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '5px', marginTop: '5px', zIndex: 0 }}>
          <div style={{ width: '50px', height: '100px', border: '1px dashed #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: '#999', backgroundColor: '#333' }}>
            Black
          </div>
          <div style={{ width: '50px', height: '100px', border: '1px dashed #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: '#999', backgroundColor: '#9e3d45' }}>
            Red
          </div>
          <div style={{ width: '50px', height: '100px', border: '1px dashed #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: '#999', backgroundColor: '#2b3952' }}>
            Navy
          </div>
        </div>
          ...
          <table style={{ width: '200px', marginLeft: 'auto', fontSize: '12px', textAlign: 'left', borderCollapse: 'collapse' }}>
            <tbody>
              ...
              <tr>
                ...
                <td style={{ padding: '1px 0' }}>: {header?.orderDate || '2020.09.13'}</td>
              </tr>
              <tr>
                <td style={{ padding: '1px 0' }}>EX FTY DATE</td>
                <td style={{ padding: '1px 0' }}>: {header?.exFtyDate || '2021.01.15'}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <table style={tableStyle}>
        <tbody>
          ...
          <tr>
            ...
            <td style={tdStyle}>{poDetails?.customerStyleDesc || poDetails?.customerStyle || 'Ladies Tuff Hybrid Short'}</td>
          </tr>
          <tr>
      ...
      <table style={tableStyle}>
        <tbody>
          ...
          <tr>
            ...
            <td style={tdStyle}>{processes?.washing || '成衣烫缩'}</td>
          </tr>
          <tr>
            <td style={tdStyle}>HEAT TRANSFER</td>
            <td style={tdStyle}>{processes?.heatTransfer || '有'}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div style={{ marginTop: '40px', marginBottom: '40px', marginLeft: '60px', fontSize: '36px', color: '#e60000', fontWeight: 'bold', fontFamily: 'Arial, sans-serif' }}>
        {remark || (header?.factoryNumber?.includes('A') ? 'PTCOC270A小单数量' : 'PTCOC270大单数量')}
      </div>
      ...
      <div style={{ position: 'absolute', bottom: '20px', left: '40px', right: '40px', fontSize: '10px', color: '#666', borderTop: '1px solid #eee', paddingTop: '10px', display: 'flex', justifyContent: 'space-between' }}>
        <span>{footerStatus?.includes('Status:') ? footerStatus : `Status: T1    ${footerStatus || 'EDIT BY MSOKHA DATE 2020/11/08 TIME 12:40:26 PRINT BY MLILIAN DATE 2020/11/30 TIME 15:01:35'}`}</span>
      </div>
    </div>
  );
};

const Comp02_OrderDetails_PTCOC270_270A = ({ data, extraction }) => {
...
export default Comp02_OrderDetails_PTCOC270_270A;
